...
$logged = 0;
$check_account_stmt = $db->prepare('SELECT * FROM users WHERE username=?,password=? ');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_REQUEST['username']) && $_REQUEST['pwd'] && $_REQUEST['rememberMe']) {
        $username = $_REQUEST['username'];
        $password = $_REQUEST['pwd'];
        $remember = isset($_REQUEST['rememberMe']) ? "on" : "off";
        $check_account_stmt->execute(array($username, $password));
        $rows = $check_account_stmt->fetchAll();
        if (count($rows) != 0) {
    ...
    if (isset($_REQUEST['title']) && isset($_REQUEST['body']) && isset($_SESSION)) {
        ...
        $body = $_REQUEST['body'];
        $author = $_SESSION['username']['fullname'];
...
<div class="twocols" style="display: <?= $logged == 0 ? '' : 'none' ?>">
...
<div class="post_panel" style="display: <?= $logged == 1 ? '' : 'none' ?>">
